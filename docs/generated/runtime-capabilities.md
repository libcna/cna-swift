...
| Audio managed enum contracts | VERIFIED_MANAGED | Exact Audio non-flags Int32 enums SoundState (Playing=0, Paused=1, Stopped=2) and AudioChannels (Mono=1, Stereo=2, no zero literal). AudioListener and AudioEmitter exist as pure managed 3D data reproducing the pinned XACT handedness flip and hold no engine handle; no SoundEffect, SoundEffectInstance, Cue, playback, mixing, XACT interop, or audio device claim, and the audio backend remains NULL. |
| CLR property accessor projection | VERIFIED_MANAGED | One accessor-level rule for every CLR property: the getter keeps Swift property or subscript syntax and gains `get throws` exactly when the XNA getter is fallible; a setter Swift cannot express becomes `Set<PropertyName>` carrying the setter's own `throws`. Fallibility is derived from the CIL of the seven registered assemblies, pinned and hash-checked -- 114 fallible getters and 113 fallible setters of 840 properties. A language mapping only; no runtime, device or native claim. |
| AudioEmitter managed 3D data | VERIFIED_MANAGED | Pinned XACT_EMITTER_DATA storage, the FlipHandedness involution and its negative-zero default `Position.Z`/`Velocity.Z`, and the exact ordered `bge.un` DopplerScale boundary where NaN and -0.0 are accepted and every ordered negative throws. No XACT engine, voice, listener pairing, or audio device claim. |
