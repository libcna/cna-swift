...
later**, qualified against `0.21.0`. Foundation Milestones 1 through 65 are
...
TARGET_TYPES=166               TARGET_MEMBERS=2065
COMPLETE_TYPES=160             PARTIAL_TYPES=6      MISSING_TYPE=91
MISSING_MEMBER=49              TOTAL_DIAGNOSTICS=144
ALLOWLIST_ENTRIES=0            UNMEASURED_STRUCTURAL_CATEGORY=0
NONDERIVABLE_UNSEALED_CLASSES=0    PENDING_BCL_BASE_TYPES=4
XNA_RESOURCE_STRING_PROJECTIONS=59 API_COMPAT_SELF_TESTS=2426
```

**Every remaining diagnostic is an absence.** Three categories are non-zero —
`MISSING_TYPE=91`, `MISSING_MEMBER=49`, and `OVERLOAD_MAPPING_MISMATCH=4`,
...
```text
BOUND_FUNCTIONS=123  ROUTE_PAIRINGS=123  PROTOTYPE_TYPE_POSITIONS=441
CANONICAL_DECLARATION_CHECKS=441  C_SWIFT_MEASUREMENTS=441
LAYOUTS=41  LAYOUT_FIELDS=336  CALLBACKS=6  CONSTANTS=221  SCALAR_FACTS=3
MISSING_HEADER_SYMBOLS=0  MISSING_LIBRARY_SYMBOLS=0  ABI_MISMATCHES=0
NATIVE_ABI_MUTATIONS=14  CAUGHT=14  SURVIVORS=0
PROJECTION_MUTATIONS=177  LAST_FULL_RUN=137  CAUGHT=135  REPLACED_NO_OPS=2
```
...
(`AUDIT_SELF_TESTS=80`, `RESOURCE_STRINGS_REPRODUCED=59`). The BCL authority
