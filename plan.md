...
later**, qualified against `0.21.0`. Foundation Milestones 1 through 66 are
...
three remaining render-target members, and `TextureCollection` with the three
bound-state checks that had nothing to check until it existed.
...
TARGET_TYPES=167               TARGET_MEMBERS=2069
COMPLETE_TYPES=161             PARTIAL_TYPES=6      MISSING_TYPE=90
MISSING_MEMBER=47              TOTAL_DIAGNOSTICS=141
ALLOWLIST_ENTRIES=0            UNMEASURED_STRUCTURAL_CATEGORY=0
NONDERIVABLE_UNSEALED_CLASSES=0    PENDING_BCL_BASE_TYPES=4
XNA_RESOURCE_STRING_PROJECTIONS=62 API_COMPAT_SELF_TESTS=2426
```

**Every remaining diagnostic is an absence.** Three categories are non-zero —
`MISSING_TYPE=90`, `MISSING_MEMBER=47`, and `OVERLOAD_MAPPING_MISMATCH=4`,
...
```text
BOUND_FUNCTIONS=125  ROUTE_PAIRINGS=125  PROTOTYPE_TYPE_POSITIONS=451
CANONICAL_DECLARATION_CHECKS=451  C_SWIFT_MEASUREMENTS=451
LAYOUTS=42  LAYOUT_FIELDS=341  CALLBACKS=6  CONSTANTS=221  SCALAR_FACTS=3
MISSING_HEADER_SYMBOLS=0  MISSING_LIBRARY_SYMBOLS=0  ABI_MISMATCHES=0
NATIVE_ABI_MUTATIONS=14  CAUGHT=14  SURVIVORS=0
PROJECTION_MUTATIONS=189  LAST_FULL_RUN=137  CAUGHT=135  REPLACED_NO_OPS=2
```
...
(`AUDIT_SELF_TESTS=80`, `RESOURCE_STRINGS_REPRODUCED=62`). The BCL authority
